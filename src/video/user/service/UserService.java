...
public class UserService implements AppService {
        ...
        while (true) {
            ...
            switch (selection) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    return; // start() 메서드를 강제 종료해서 초기 화면으로 돌아갈 수 있도록 처리 하겠다. start 메서드 자체가 끝나고 가장 밖에 있는게 종료됨 강제종료.
                default:
    ...
    // 이름을 입력 받아 해당 이름이 포함된 객체 리스트 리턴하는 기능.
    ...
    // 회원 추가 비지니스 로직
    // 비지니스 로직: 요청에 따른 데이터의 생성, 조회, 저장, 변경 등의 로직.
    private void join() {
    ...
    private void deleteUser() {
        ...
        if (users.size() > 0) {
            ...
            System.out.println("\n### 탈퇴할 회원의 번호를 입력하세요.");
            int delUserName = inputInteger(">>> ");

            if(userNums.contains(delUserName)) {
                User delUser = userRepository.deleteUser(delUserName);
                System.out.printf("\n### %s[%s] 님의 회원정보가 정상적으로 삭제되었습니다.\n"
                        ,delUser.getUserName(), delUser.getPhoneNumber());
            } else {
                System.out.println("\n### 검색한 회원의 번호로만 삭제가 가능합니다.");
            }
