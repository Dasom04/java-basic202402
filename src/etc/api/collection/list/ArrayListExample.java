...
//        String name = nick[3]; (x) 배열의 방법
        ...
        System.out.println("3번 인덱스: " + name);
        ...
        // isEmpty(): 리스트가 비어있는지의 여부 확인. 비어있으면 true, 있으면 false.
        System.out.println(nick.isEmpty());

        makeLine();

        /*
        # 컬렉션 개체들은 Collections의 기능을 사용할 수 있습니다.
        Collections는 컬렉션 객체들의 부가 기능들을 지원합니다.
         */

        List<Integer> score = new ArrayList<>();  // int -> Integer 로 써야함, cha -> Character
        score.add(65);
...
//        Collections.reverse(score); // 역순 배치
        Collections.sort(score, Collections.reverseOrder()); // 리스트, 매개값 (처음부터 내림차 저렬 하는 방법)
        System.out.println(score);
